...
const getAll = async (id: string): Promise<Array<THotelTour>> => {
	const response = await fetch(`${import.meta.env.VITE_API_URI}/api/tour/${id}/hotel`, {
		method: 'GET'
	})
...
const create = async (data: TAddHotelTourForm): Promise<void> => {
	await fetch(`${import.meta.env.VITE_API_URI}/api/tour/${data.tourId}/hotel/${data.hotelId}`, {
		method: 'POST',
...
const edit = async (data: TEditHotelTourForm): Promise<void> => {
	await fetch(
		`${import.meta.env.VITE_API_URI}/api/tour/${data.tourId}/hotel/${data.hotelId}`,
		{
			method: 'PUT',
...
const deleteHotelTour = async (baseId: string, id: string) => {
	await fetch(`${import.meta.env.VITE_API_URI}/api/tour/${baseId}/hotel/${id}`, {
		method: 'DELETE'
	})
}
